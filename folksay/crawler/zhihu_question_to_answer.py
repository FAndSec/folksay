...
# Ref: https://github.com/visionshao/-zhihu-crawl-/blob/master/crawl/zhihui_scrapy_single_QA.py
...
# Given next_url, return list of answers
def getAnswers(next_url):
    count = 0
    allAnswer = []
    for url in next_url:
...
if __name__ == '__main__':
    ...
    urlPrefix = 'https://www.zhihu.com/api/v4/questions/'
    urlPostfix = '/answers?include=data%5B%2A%5D.is_normal%2Cadmin_closed_comment%2Creward_info%2Cis_collapsed%2Cannotation_action%2Cannotation_detail%2Ccollapse_reason%2Cis_sticky%2Ccollapsed_by%2Csuggest_edit%2Ccomment_count%2Ccan_comment%2Ccontent%2Ceditable_content%2Cvoteup_count%2Creshipment_settings%2Ccomment_permission%2Ccreated_time%2Cupdated_time%2Creview_info%2Crelevant_info%2Cquestion%2Cexcerpt%2Crelationship.is_authorized%2Cis_author%2Cvoting%2Cis_thanked%2Cis_nothelp%3Bdata%5B%2A%5D.mark_infos%5B%2A%5D.url%3Bdata%5B%2A%5D.author.follower_count%2Cbadge%5B%2A%5D.topics&limit=5&offset=0&sort_by=default'
    print("running.")
    allURLs = []
    survey = MyDatabase()
    allQuestions = survey.retrieveAllQuestionIDs()
    survey.closeDB()
    survey = MyDatabase()
    survey.createTableAnswer()
    for id in allQuestions:
        url = urlPrefix + str(id) + urlPostfix
        allURLs.append(url)
        allAnswer = getAnswers([url])
        if allAnswer == None:
            survey.closeDB()
            break
